...
|------|---------|-----------|--------|
| Aug 24 | **LOCAL REVIEW ONLY — NOT DEPLOYED:** Rene Phase 2 least-privilege renewal-book identity/undated exact-ID reads plus a separate `/mcp/rene-phase2/` registry and opaque credential that cannot access generic MCP/REST; inspect/validate/prepare/verify/record-sent/fill/lookup boundary; terminal no-replay rollback receipts; unseeded reviewed policy records; actual visible-text verification of prepare-only final-number PDF with no source lifecycle effect | `rene_auth.py`, `rene_mcp_server.py`, `rene_views.py`, `mcp.py`, `rene_phase2.py`, `renewal_book_service.py`, `0097`, tests, `docs/RENE_PHASE2_CRM_BOUNDARY.md` | local branch |
| Jul 27 | fix(contract): safe Beat Breeze service-location writes — expose `service_type` and service-location rows to API/MCP, preserve row IDs, make destructive replacement explicit, and block PDFs when flat per-zone pricing contradicts `contract.value`. Regression for HK-CT26868 (The Theodore Hanoi) | `serializers.py`, `mcp.py`, `views.py`, `contract_service_locations.py`, `ContractForm.tsx`, tests | `921ef9f` |
| Jun 19 | fix(quote-pdf): prorated/annex quotes no longer print 'Total (per year)' — a 12-month quote always labelled the total '(per year)', so the Hyatt Danang annex rendered 'Total (per year): USD 170.02' against its 580/yr rate (Theo's per-year-template bug; blocked Hyatt procurement). Now detects proration (notes / line descriptions) → 'Total:'; normal annual quotes keep Pom's '(per year)'. No schema change | `quote_pdf.py` | `bcace11` |
| Jun 19 | feat(company): funnel-sourced contracted_product + contracted_zone_count + contracted_synced_at — company-level reporting of BB vs SYB + contracted zone count, sourced from the renewal funnel, DISTINCT from music_zone_count + the Soundtrack-synced Zone/ContractServiceLocation tables (never drives the rendered PDF). Backfilled via Cira. Closes the 'CRM blind to Beat Breeze' gap for reporting | `models.py`, `serializers.py`, `0093` | `c18ef93` |
