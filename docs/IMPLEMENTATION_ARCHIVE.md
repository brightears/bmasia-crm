...
> **Note**: This file contains the complete implementation history for BMAsia CRM. It is NOT automatically loaded by Claude Code. For a quick reference, see `.claude/rules/implementation-history.md`.

---

### Feb 22, 2026 - Contract PDF Footer Page Overflow Fix

**Problem**: Contract PDF footer (BMAsia entity name + full address + phone) was embedded as a flowable element in the content stream. When content filled a page almost completely, the footer text wrapped and spilled onto a blank next page — creating an ugly near-empty final page with just "2153 3520" (the tail end of the phone number).

**Root Cause**: All 4 contract PDF code paths used `Paragraph` flowable elements for the footer. Unlike the invoice PDF, which already used a canvas-based footer callback (`draw_invoice_footer`), contracts let ReportLab paginate the footer text like normal content.

**Fix**: Applied the same canvas-based footer pattern from invoices to all 4 contract PDF types:
1. Increased `bottomMargin` from `0.4*inch` to `0.75*inch` to reserve space for the footer
2. Added `draw_contract_footer` callback (closure over entity vars) that draws:
   - Gray separator line at `0.65*inch` from bottom
   - Bold entity name centered at `0.48*inch` (7pt DejaVuSans-Bold)
   - Address + phone centered at `0.35*inch` (7pt DejaVuSans)
3. Removed footer flowable elements (Spacer, HRFlowable, Paragraph) from content stream
4. Changed `doc.build(elements)` → `doc.build(elements, onFirstPage=draw_contract_footer, onLaterPages=draw_contract_footer)`

**4 locations fixed in `crm_app/views.py`**:
| PDF Type | What changed |
|----------|-------------|
| Standard (template mode) | Removed footer flowables, added callback to build() |
| Standard (non-template) | Removed footer from `signature_elements` KeepTogether group, added callback |
| Master agreement | Added callback definition, removed footer flowables, updated build() |
| Participation agreement | Added callback definition, removed footer flowables, updated build() |

**Lesson**: Never use flowable elements for PDF footers — they're part of the content stream and can split across pages. Always use canvas-based `onPage` callbacks for footers (and headers). The invoice PDF already had this pattern right.

**Commit**: `0bc3dcfd`

---

### Feb 22, 2026 - Contract PDF Validity Date Overflow

**Problem**: The Validity column in the PRINCIPAL TERMS metadata table showed "Feb 15, 2026 - Feb 15, 2028" as a plain string that overflowed the 1.8" column width.

**Fix**:
- Changed Validity cell from plain string to `Paragraph` with `<br/>` — "Feb 15, 2026<br/>to Feb 15, 2028" (two lines)
- Widened Validity column from 1.8" to 2.4" (stole from Date and Status columns)
- Applied to all 3 PDF types: standard, master agreement, participation

**Files Modified**: `crm_app/views.py` (3 locations)

**Commit**: `de3cfc95`

---

### Feb 22, 2026 - Status-Based Contract Edit Restrictions

**Problem**: Any contract could be edited regardless of status. Once signed (Active), terms are binding and shouldn't be silently changed. Renewed/Expired/Cancelled are historical records.

**Edit Rules**:
- **Draft + Sent**: fully editable (pre-signing, corrections are normal)
- **Active**: locked — "This contract is active and cannot be edited. To change terms, create a new contract or amendment."
- **Renewed**: locked — "The renewal contract is the current version."
- **Expired / Cancelled**: locked — "This is a historical record."

**Implementation (frontend-only)**:
- `LOCKED_STATUSES = ['Active', 'Renewed', 'Expired', 'Cancelled']` constant in `Contracts.tsx`
- **Contracts.tsx 3-dot menu**: Edit MenuItem disabled for locked statuses, shows "(locked)" label
- **ContractDetail.tsx**: Info Alert banner at top of dialog content explaining why contract is locked. Edit button hidden for locked statuses. Status-specific messaging per status.
- No backend changes — API is internal-only, frontend enforcement sufficient for now

**Files Modified**:
- `bmasia-crm-frontend/src/pages/Contracts.tsx` — `LOCKED_STATUSES` const, disabled Edit menu item
- `bmasia-crm-frontend/src/components/ContractDetail.tsx` — Info Alert banner, conditional Edit button

**Commit**: `29081fb0`

---

### Feb 22, 2026 - Contract Editing: 4 Bug Fixes

**Bug 1 — Template dropdown shows wrong template in edit mode**:
- **Root cause**: `loadContractTemplates()` filtered by `is_active: true` — deactivated templates wouldn't appear. Also, contracts created before template system have `preamble_template = null`.
- **Fix**: Load all templates (removed `is_active` filter). Inactive templates show "(inactive)" label, disabled for create mode but selectable in edit mode.

**Bug 2 — No save confirmation**:
- **Root cause**: `handleSubmit()` called `onSave()` + `onClose()` — form closed silently.
- **Fix**: Added success Snackbar in `Contracts.tsx` `handleContractSave()` — "Contract created/updated successfully". Uses existing `success` state + Alert pattern.

**Bug 3 — Can't save older contracts**:
- **Root cause**: Template validation `if (!selectedTemplate)` at line 720 blocked ALL contracts without templates — including all contracts created before the template system.
- **Fix**: Changed to `if (!selectedTemplate && mode === 'create')` — only required for new contracts.
- **Secondary fix**: Improved DRF error parsing — extracts field-level errors (`{ field_name: ['error'] }`) instead of showing generic "Failed to save contract".

**Bug 4 — Service locations show wrong platforms (2 SYB instead of 1 SYB + 1 BB)**:
- **Root cause**: Timing bug in `populateForm()`. `syncLocationsFromLineItems()` relied on `useEffect([lineItems])` which fired while `isFormInitialized.current` was still `false` (set via `setTimeout`), so it returned early. The ref became `true` after `lineItems` was already set, so the useEffect never re-fired. Sync never ran in edit mode.
- **Fix**: Compute synced locations directly in `populateForm()` before calling `setState()`. Prepare line items → load DB locations → call `syncLocationsFromLineItems()` inline → set both to state.

**Files Modified**:
- `bmasia-crm-frontend/src/components/ContractForm.tsx` — All 4 fixes
- `bmasia-crm-frontend/src/pages/Contracts.tsx` — Save confirmation Snackbar

**Commit**: `93bfeb5d`

---

### Feb 20-21, 2026 - Flexible Contract Duration + PDF Bug Fixes

**Context**: Nikki's feedback — CRM assumed all subscriptions are 1 year. Customers may want 6-month trials, 2-year or 3-year deals. Implemented duration support across quotes, contracts, and PDFs, then fixed 3 bugs found during testing.

**Part 1 — Duration Feature** (commit `7550e654`):
- Quote model: `contract_duration_months` PositiveIntegerField (default=12). Migration `0075`
- QuoteForm: duration presets dropdown (6mo, 1yr, 2yr, 3yr, Custom with free-text months)
- Quote PDF: 4th metadata column "Contract Duration" + "Total Contract Value (N years)" line for multi-year
- ContractForm: `durationMonths` state with presets, replaces hardcoded `calculateEndDate()`. Edit mode derives duration from existing dates
- Contract PDF: `_format_duration()` helper replaces hardcoded "12 months"
- Quote→Contract: `handleQuoteChange()` transfers `contract_duration_months` to `durationMonths`

**Part 2 — Bug Fixes** (commits `e85327d2`, `0bec9657`, `a214abc5`, `ef616ac3`):

**Bug 1 — `_format_duration()` off-by-one** (`views.py:94`, `models.py:889`, `admin.py:989`):
- `if end_date.day >= start_date.day:` → `if end_date.day > start_date.day:`
- Feb 19→Feb 19 (24 months) was reporting as "2 years and 1 month"
- Also: `'12 months'` → `'1 year'` for consistency

**Bug 2 — Double currency "THB THB"** in template-based contracts:
- Root cause: `{{value}}` = "THB 21,000.00" (includes currency), template text had `{{currency}} {{value}}`
- Fix: Added `{{value_amount}}` variable (number only, no currency). Templates use `{{currency}} {{value_amount}}`

**Bug 3 — "Term: Annually"** in template-based contracts:
- Root cause: No `{{duration}}` template variable existed; template author hardcoded "Annually"
- Fix: Added `{{duration}}` variable that resolves to e.g., "2 years"

**Bug 4 — Annual-only pricing** in hardcoded clause path:
- Added "per year" label + "Total Contract Value (N years)" line for multi-year contracts
- Both line-items and legacy paths in hardcoded Total Cost clause (views.py ~line 2206)

**Bug 5 — Master agreement double currency** (`views.py ~line 3077`):
- `f"{currency_symbol}{contract.value:,.2f} {contract.currency}"` had both `currency_symbol` AND `.currency`
- Fix: removed trailing ` {contract.currency}`. Also added TCV row for multi-year

**New Template Variables** (added to `_substitute_template_variables()` in views.py):

| Variable | Value | Notes |
|----------|-------|-------|
| `{{duration}}` | "2 years" | From `_format_duration()` |
| `{{value_amount}}` | "21,000.00" | Number only, no currency |
| `{{annual_value}}` | "THB 21,000.00" | Alias for `{{value}}` |
| `{{total_contract_value}}` | "THB 42,000.00" | Always populated (falls back to annual for 1yr) |
| `{{total_contract_value_amount}}` | "42,000.00" | Number only |
| `{{billing_note}}` | "Invoiced annually at THB 21,000.00 + 7% VAT per year." | Multi-year only; empty for 1yr. VAT suffix only for Thailand entity |

**All 4 Templates Updated** (user manually updated via Contract Templates admin):

| Template | Entity | Product | Key Differences |
|----------|--------|---------|-----------------|
| 1 | BMAsia Thailand | SYB | + 7% VAT in line 5 |
| 2 | BMAsia Limited | SYB | No VAT in line 5 |
| 3 | BMAsia Thailand | Beat Breeze | + 7% VAT, preamble had "12 months" → `{{duration}}` |
| 4 | BMAsia Limited | Beat Breeze | No VAT, preamble had "12 months" → `{{duration}}` |

Common changes across all 4: line 4 `{{billing_frequency}}` → `{{duration}}`, line 5 `{{currency}} {{value}}` → `{{total_contract_value}}` + `{{billing_note}}`

**Files Modified**:
- `crm_app/models.py` — Quote.contract_duration_months, off-by-one fix in Contract.monthly_value
- `crm_app/views.py` — `_format_duration()` + `_format_duration_from_months()` helpers, 6 new template variables, hardcoded clause multi-year TCV, master agreement fixes
- `crm_app/admin.py` — off-by-one fix in monthly_value_display
- `crm_app/serializers.py` — contract_duration_months in QuoteSerializer
- `crm_app/migrations/0075_quote_contract_duration.py` — New migration
- `bmasia-crm-frontend/src/types/index.ts` — Quote.contract_duration_months
- `bmasia-crm-frontend/src/components/QuoteForm.tsx` — Duration selector UI
- `bmasia-crm-frontend/src/components/ContractForm.tsx` — Duration presets, enhanced calculateEndDate()
- `bmasia-crm-frontend/src/components/ContractTemplateForm.tsx` — 6 new TEMPLATE_VARIABLES entries
