...
class QuoteSendReceiptUse(models.Model):
    """One durable, idempotent use of a quote-attestor-signed send receipt.

    Mirror of ContractSendReceiptUse: unique request key, nonce and receipt
    digest prevent replay across records; the quote write and this row commit
    in the same transaction.
    """

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    quote = models.ForeignKey(
        'Quote', on_delete=models.PROTECT, related_name='send_receipt_uses',
    )
    request_key = models.CharField(max_length=256, unique=True)
    nonce = models.UUIDField(unique=True)
    receipt_sha256 = models.CharField(max_length=64, unique=True)
    ...
    def __str__(self):
        return f"{self.quote_number} sent {self.sent_date} ({self.mailbox})"
